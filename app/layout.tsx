...
import { getActiveDomain, getDomainConfig, DOMAINS } from "@/lib/domain";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    keywords,
    authors: [{ name: "चालू घडामोडी", url: config.baseUrl }],
    creator: "चालू घडामोडी",
    publisher: "चालू घडामोडी",
    category: "education",
    openGraph: {
      title: config.title,
      description: "MPSC, तलाठी, पोलीस भरती, रेल्वे, SSC GD — सर्व परीक्षांसाठी मोफत मराठी चालू घडामोडी, मासिक PDF, रोजच्या टेस्ट.",
      locale: "mr_IN",
      type: "website",
      url: config.baseUrl,
    ...
    twitter: {
      ...
      images: [`${config.baseUrl}/og-image.png`],
    },
    robots: {
      index: true,
      follow: true,
    ...
    other: {
      ...
      "citation_publisher": domain,
      // Regional signals
      "geo.region": "IN-MH",
      "geo.country": "IN",
      "language": "Marathi",
      // AI discovery
      "llms-txt": `${config.baseUrl}/llms.txt`,
    },
  };
}
...
export default async function RootLayout({
  ...
  const websiteSchema = {
    ...
    url: config.baseUrl,
    description: domain === DOMAINS.MPSC
      ? "MPSC Current Affairs — मराठी विद्यार्थ्यांसाठी मोफत चालू घडामोडी, मासिक PDF, आणि टेस्ट."
  ...
  const educationalOrgSchema = {
    ...
    name: config.brandName,
    alternateName: domain === DOMAINS.MPSC ? "MPSC Current Affairs" : "Chalu Ghadamodi",
    url: config.baseUrl,
    logo: `${config.baseUrl}/og-image.png`,
    description: domain === DOMAINS.MPSC
      ? "MPSC Current Affairs — महाराष्ट्रातील स्पर्धा परीक्षा विद्यार्थ्यांसाठी मोफत मराठी चालू घडामोडी, मासिक PDF, आणि सराव टेस्ट."
...
fbq('track', 'PageView');`,
        ...
        <link rel="alternate" hrefLang="mr" href={config.baseUrl} />
        <link rel="alternate" hrefLang="x-default" href={config.baseUrl} />
        {/* AI/LLM discovery */}
        <link rel="llms-txt" href={`${config.baseUrl}/llms.txt`} />
