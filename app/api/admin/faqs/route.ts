...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const row = await db
      ...
      .bind(id, body.question, body.answer, body.published !== undefined ? (body.published ? 1 : 0) : 1, body.display_order ?? 0, now, now)
      .first();
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    vals.push(id);
    const sql = `UPDATE faqs SET ${sets.join(", ")} WHERE id = ? RETURNING *`;
