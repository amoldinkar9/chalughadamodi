...
export async function POST(req: NextRequest) {
  ...
  const row = await db
    .prepare("INSERT INTO faqs (id, question, answer, published, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *")
    .bind(id, body.question, body.answer, body.published ?? 1, body.display_order ?? 0, now, now)
    .first();
...
export async function PUT(req: NextRequest) {
  ...
  vals.push(id);
  const row = await db.prepare(`UPDATE faqs SET ${sets.join(", ")} WHERE id = ? RETURNING *`).bind(...vals).first();
