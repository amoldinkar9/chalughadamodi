...
export async function POST(req: NextRequest) {
  ...
  const row = await db
    .prepare("INSERT INTO magazines (id, month, image_url, pdf_url, published, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *")
    .bind(id, body.month, body.image_url ?? "", body.pdf_url ?? "", body.published ?? 1, body.display_order ?? 0, now, now)
    .first();
...
export async function PUT(req: NextRequest) {
  ...
  vals.push(id);
  const row = await db.prepare(`UPDATE magazines SET ${sets.join(", ")} WHERE id = ? RETURNING *`).bind(...vals).first();
