...
export async function POST(req: NextRequest) {
  ...
  const row = await db
    ...
    .bind(id, body.name, body.initials, body.exam, body.quote, body.published ?? 1, body.display_order ?? 0, now, now)
    .first();
...
export async function PUT(req: NextRequest) {
  ...
  vals.push(id);
  const row = await db.prepare(`UPDATE testimonials SET ${sets.join(", ")} WHERE id = ? RETURNING *`).bind(...vals).first();
