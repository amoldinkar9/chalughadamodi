...
export async function POST(req: NextRequest) {
  ...
  const row = await db
    .prepare("INSERT INTO announcements (id, title, image_url, backlink, published, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *")
    .bind(id, body.title, body.image_url ?? "", body.backlink ?? "", body.published ?? 1, body.display_order ?? 0, now, now)
    .first();
...
export async function PUT(req: NextRequest) {
  ...
  vals.push(id);
  const row = await db.prepare(`UPDATE announcements SET ${sets.join(", ")} WHERE id = ? RETURNING *`).bind(...vals).first();
